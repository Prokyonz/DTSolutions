...
SELECT LOWER(NEWID()) 'Id', M.* FROM (        
	...
	LEFT JOIN KapanMappingMaster KMM ON KMM.PurchaseMasterId = T.PurId AND isnull(KMM.TransferType,'')='' 
	LEFT JOIN KapanMaster KM ON KM.Id = KapanId AND KM.CompanyId=@CompanyId
) M        
