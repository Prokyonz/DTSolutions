...
namespace DiamondTrade.API.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        public async Task<Response<dynamic>> GetStockReport(string CompanyId, string financialYearId)
        {
            try
            {
                // Fetch stock and number reports asynchronously
                var stockReportTask = _accountToAssortMaster.GetStockReportSummaryAsync(CompanyId, financialYearId);
                ...
                // Get the results of both tasks
                var stockReport = await stockReportTask;
                var numberReport = await numberReportTask;

                // Calculate stats for both reports
                var stockReportStats = CalculateReportStats(stockReport);
                var numberReportStats = CalculateReportStats(numberReport);

                // Create StockReportMasterGrid objects
        ...
        private (decimal Rate, decimal TotalWeight, decimal TotalAmount) CalculateReportStats(IEnumerable<StockReportSummayGrid> report)
        {
            return (report.First().Rate, report.First().NetWeight, report.First().Amount);
        }
